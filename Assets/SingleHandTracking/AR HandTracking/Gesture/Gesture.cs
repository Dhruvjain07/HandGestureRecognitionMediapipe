...
public class Gesture : MonoBehaviour {
  ...
  #region Singleton
    ...
    // return true if 1 is higher than 2
    ...
    public bool isFive(){
        if (handQueue.Count < 5) return false;
        ARHand[] hands = handQueue.ToArray();
        if(angleFingerStraight(hands[4],1) && angleFingerStraight(hands[4],5) && angleFingerStraight(hands[4],9) && angleFingerStraight(hands[4],13) && angleFingerStraight(hands[4],17))
            return true;
        return false;
    }
